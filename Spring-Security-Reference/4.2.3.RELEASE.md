...
Spring Security은 여러가지 방법으로 적용할 수 있다. Spring Security 메인 페이지에서 패키지를 다운로드하거나 Maven Central repository(또는 스냅샷 및 중요 배포를 위한 Spring Maven repository)에서 개별 jar 파일들을 다운로드 할 수 있다. 아니면 직접 소스에서 프로젝트를 빌드할 수도 있다.
