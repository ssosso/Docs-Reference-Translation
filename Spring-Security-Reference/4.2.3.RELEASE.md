...
- [5.1. 웹 Security Java 설정](#51-웹-security-java-설정)
   ...
   - [5.1.2. AbstractSecurityWebApplicationInitializer (Spring이 없는 경우)](#512-abstractsecuritywebapplicationinitializer-spring이-없는-경우)
...
- [5.4. 승인 요청 (Authorize Requests)](#54-승인-요청-authorize-requests)
- [5.5. 로그아웃](#55-로그아웃)
...
- [5.9. 사후 처리하는 설정 객체](#59-사후-처리하는-설정-객체)
- [5.10. 사용자 정의 DSL](#510-사용자-정의-dsl)
...
- [6.3. 고급 웹 기능](#63-고급-웹-기능)
   ...
   - [6.3.3. 세션 관리하기](#633-세션-관리하기)
      - [Timeout 방지하기](#timeout-방지하기)
      - [동시 세션 제어하기](#동시-세션-제어하기)
      - [세션 강제고정 공격 방어하기](#세션-강제고정-공격-방어하기)
...
- [18.3. CSRF 방어를 사용하는 경우](#183-csrf-방어를-사용하는-경우)
   ...
   - [18.3.2. CSRF와 상태 비저장 브라우저 어플리케이션(Stateless Browser Application)](#1832-csrf와-상태-비저장-브라우저-어플리케이션stateless-browser-application)
...
- [18.5. CSRF 주의사항](#185-csrf-주의사항)
   ...
   - [18.5.2. Logging In](#1852-logging-in)
   - [18.5.3. Logging Out](#1853-logging-out)
   - [18.5.4. Multipart(파일 업로드)](#1854-multipart파일-업로드)
      - [Spring Security에 앞서 MultipartFilter 적용하기](#spring-security에-앞서-multipartfilter-적용하기)
...
- [30.6. CSRF메타태그 태그 ```csrfMetaTags```](#306-csrf메타태그-태그-csrfmetatags)
...
- [32.3. CAS 클라이언트 설정](#323-cas-클라이언트-설정)
   ...
   - [32.3.3. CAS로 상태 비저장(Stateless) 서비스 인증하기](#3233-cas로-상태-비저장stateless-서비스-인증하기)
      - [Ticket이 부여된 Proxy를 얻기 위해 CAS 설정하기](#ticket이-부여된-proxy를-얻기-위해-cas-설정하기)
      - [Proxy Ticket 사용하여 상태 비저장(Stateless) 서비스 부르기](#proxy-ticket-사용하여-상태-비저장stateless-서비스-부르기)
...
- [37.5. Spring MVC와 CSRF와의 통합](#375-spring-mvc와-csrf와의-통합)
   ...
   - [37.5.2. CSRF 토큰 확인하기](#3752-csrf-토큰-확인하기)
...
인증 단계에서 Spring Security는 광범위한 인증 모델을 지원한다. 이러한 인증 모델의 대부분은 제 3자에 의해서 제공되거나 인터넷 엔지니어링 태스크 포스(Internet Engineering Task Force) 관련 표준 기관에 의해서 개발된다. 그에 더해 Spring Security는 자체적인 인증 기능들을 제공하기도 한다. 특히, Spring Security는 현재 아래와 같은 기술들과의 인증 통합을 지원한다.
...
Spring Security은 여러가지 방법으로 적용할 수 있다. Spring Security 메인 페이지에서 패키지를 다운로드하거나 Maven Central repository(또는 스냅샷 및 마일스톤[^1] 배포를 위한 Spring Maven repository)에서 개별 jar 파일들을 다운로드 할 수 있다. 아니면 직접 소스에서 프로젝트를 빌드할 수도 있다.

[^1]: 마일스톤(Milestone)은 프로젝트 진행 과정에서 어떤 중요한 시점을 의미한다. 따라서 마일스톤 배포라고 하면 정식 배포 버전은 아니지만 어떠한 중요한 업그레이드를 위한 배포라고 생각하면 될 듯 하다. -옮긴이
...
- [#4102](https://github.com/spring-projects/spring-security/issues/4102) - Custom default configuration in ```WebSecurityConfigurerAdapter```. See [Section 5.10, “Custom DSLs”](https://docs.spring.io/spring-security/site/docs/4.2.3.RELEASE/reference/htmlsingle/#jc-custom-dsls)
- [#3899](https://github.com/spring-projects/spring-security/issues/3899) - [concurrency-control@max-sessions](https://docs.spring.io/spring-security/site/docs/4.2.3.RELEASE/reference/htmlsingle/#nsa-concurrency-control-max-sessions) supports unlimited sessions.
...
| Source | Description | Guide |
  |---|---|---|
  | [Hello Spring Security](https://github.com/spring-projects/spring-security/tree/4.2.3.RELEASE/samples/javaconfig/helloworld) | Demonstrates how to integrate Spring Security with an existing application using Java-based configuration. | [Hello Spring Security Guide](https://docs.spring.io/spring-security/site/docs/4.2.3.RELEASE/guides/html5/helloworld-javaconfig.html) |
  | [Hello Spring Security Boot](https://github.com/spring-projects/spring-security/tree/4.2.3.RELEASE/samples/boot/helloworld) | Demonstrates how to integrate Spring Security with an existing Spring Boot application. | [Hello Spring Security Boot Guide](https://docs.spring.io/spring-security/site/docs/4.2.3.RELEASE/guides/html5/helloworld-boot.html) |
  | [Hello Spring Security XML](https://github.com/spring-projects/spring-security/tree/4.2.3.RELEASE/samples/xml/helloworld) | Demonstrates how to integrate Spring Security with an existing application using XML-based configuration. | [Hello Spring Security XML Guide](https://docs.spring.io/spring-security/site/docs/4.2.3.RELEASE/guides/html5/helloworld-xml.html) |
  | [Hello Spring MVC Security](https://github.com/spring-projects/spring-security/tree/4.2.3.RELEASE/samples/javaconfig/hellomvc) | Demonstrates how to integrate Spring Security with an existing Spring MVC application. | [Hello Spring MVC Security Guide](https://docs.spring.io/spring-security/site/docs/4.2.3.RELEASE/guides/html5/hellomvc-javaconfig.html) |
  | [Custom Login Form](https://github.com/spring-projects/spring-security/tree/4.2.3.RELEASE/samples/javaconfig/form) | Demonstrates how to create a custom login form. | [Custom Login Form Guide](https://docs.spring.io/spring-security/site/docs/4.2.3.RELEASE/guides/html5/form-javaconfig.html) |
...
#### 5.1.2. AbstractSecurityWebApplicationInitializer (Spring이 없는 경우)
```java
...
#### 5.6.2. JDBC 인증 (JDBC Authentication)
```java
@Autowired
...
#### 5.6.3. LDAP 인증 (LDAP Authentication)
```java
@Autowired
private DataSource dataSource;
...
##### 사용자 지정 AuthenticationEntryPoint 설정하기
### 6.4. 메서드 Security
...
#### 18.3.2. CSRF와 상태 비저장 브라우저 어플리케이션(Stateless Browser Application)
...
##### Spring Security에 앞서 MultipartFilter 적용하기
...
#### 32.3.3. CAS로 상태 비저장(Stateless) 서비스 인증하기
##### Ticket이 부여된 Proxy를 얻기 위해 CAS 설정하기
##### Proxy Ticket 사용하여 상태 비저장(Stateless) 서비스 부르기
